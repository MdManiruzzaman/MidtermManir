...
public class ConnectToSqlDB {
    ...
    public void createTableFromStringToMySql2(String tableName, String columnName1,String columnName2){
        try {
            ...
            ps.executeUpdate();
            ps = connect.prepareStatement("CREATE TABLE `"+tableName+"` (`ID` int(11) NOT NULL AUTO_INCREMENT,`"+columnName1+"` varchar(100) DEFAULT NULL,`"+columnName2+"` varchar(100) DEFAULT NULL,  PRIMARY KEY (`ID`) );");
            ps.executeUpdate();
        } catch (IOException e) {
            e.printStackTrace();
    ...
    public void insertDataFromArrayListToSqlTable(List<Object> list, String tableName, String columnName)
    {
        try {
            ...
            ps = connect.prepareStatement("DROP TABLE IF EXISTS `"+tableName+"`;");
            ps.executeUpdate();
            ps = connect.prepareStatement("CREATE TABLE `"+tableName+"` (`ID` int(11) NOT NULL AUTO_INCREMENT,`"+ columnName+"` varchar(100) DEFAULT NULL,  PRIMARY KEY (`ID`) );");
            ps.executeUpdate();
            for(Object ob :list){
                ps = connect.prepareStatement("INSERT INTO "+tableName+" ( "+columnName+" ) VALUES(?)");
                ps.setObject(1,ob);
    ...
    public void insertProfileToSqlTable(String tableName, String columnName1, String columnName2)
    {
        try {
            connectToSqlDatabase();
            ps = connect.prepareStatement("INSERT INTO "+tableName+" ( " + columnName1 + "," + columnName2 + " ) VALUES(?,?)");
            ps.setString(1,"Khalid Hasan Milu");
            ps.setInt(2,5555555);
            ps.executeUpdate();
